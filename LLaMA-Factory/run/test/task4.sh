...
# Dirs
mkdir -p generate/task4 generate/task4_group
mkdir -p log/task4/infer log/task4_group/infer
...
# - train_ase:        saves/task4/${model_tag}/lora/sft
# - train_ase_group:  saves/task4/${model_tag}_group/lora/sft
...
datasets=("test_ase" "test_ase_group")

for dataset in "${datasets[@]}"; do
  if [[ "$dataset" == "test_ase_group" ]]; then
    gen_dir="generate/task4_group"
    log_dir="log/task4_group/infer"
    group_suffix="_group"
  else
    gen_dir="generate/task4"
    log_dir="log/task4/infer"
    group_suffix=""
  fi
  ...
  for model_name in "${!sft_models[@]}"; do
    ...
    adapter_dir="saves/task4/${model_tag}${group_suffix}/lora/sft"
...
echo "All task4 inference tasks have been completed/submitted."