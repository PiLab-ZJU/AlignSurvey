...
# Models that have SFT adapters; base model paths
...
for model_name in "${!sft_models[@]}"; do
  ...
  # Match training output_dir exactly: saves/task2/${model_name//./_}/lora/sft
  adapter_dir="saves/task2/${model_name//./_}/lora/sft"
  save_file="generate/task2/sft_${model_tag}.jsonl"
  log_file="log/task2/infer/sft_${model_tag}.log"

  if [[ ! -d "$adapter_dir" ]]; then
    echo "Adapter directory not found: $adapter_dir. Skipping SFT inference for ${model_name}."
    continue
  fi
...
for model_name in "${nosft_models[@]}"; do
  ...
  save_file="generate/task2/nosft_${model_tag}.jsonl"
  log_file="log/task2/infer/nosft_${model_tag}.log"
